...
- 修复不提供 ADB USB 接口的旧版 DJI 模块无法切换到 iPhone/iPad 模式的问题：网络唤醒安装失败时显示能力降级提示，但不再阻断核心 USB 模式切换。
- USB 模式切换与蜂窝信号自动恢复互斥执行，避免恢复命令或模块重启打断配置写入；切换成功后重置恢复节流窗口，为拔插设备留出时间。
- iPhone/iPad 模式已保存并等待拔插时暂停蜂窝信号自动恢复，避免 `CGATT`、`COPS` 或 `CFUN` 干扰恢复 Mac 模式；设备重新接回 Mac 后仍会先执行自动恢复。
- 连接模式使用独立加载状态并优先刷新，不再因网络、流量或 eSIM 请求缓慢而将 iPhone/iPad 按钮长期禁用。
- 通话轮询短暂失败时不再强制重启已加载的后端，避免清空已检测到的来电并让通知立即消失；新增实际来电通知日志用于现场验收。
- 通知器日期解码兼容 Go 后端带微秒的 ISO 8601 时间戳，修复来电已被后端识别、但整个 `CallStatus` 解码失败而从不显示来电面板的问题。
- macOS 网络修复会同时识别 SystemConfiguration 中的隐藏 Baiwang 服务，避免已有 `en4` DHCP 服务被误判为未配置并反复创建失败。
- ECM 重新枚举到新接口时只复用 active 的模块网络服务；旧隐藏服务仍绑定 `en4` 时，会为新出现的 `en8` 等接口创建独立服务并续租 DHCP。
- 识别 macOS 自动创建的 `EG25G-QDC507` 网络服务，且未配置候选必须处于 active，避免将 inactive 的 Thunderbolt `en1` 误判为模块网卡。
- 流量统计支持仅获得全球 IPv6 的 ECM 接口，不再因 IPv4 为 `169.254/16` 就漏掉 `en8` 的收发字节；DHCP 修复仍以有效 IPv4 配置为准，避免把“有 IPv6 地址但无默认路由”误判为已联网。
- `usbnet=1` 的 ECM 模式连续两次 DHCP 失败时，自动回退到模块固定网段 `192.168.225.2/24` 与网关 `192.168.225.1`，解决该固件不下发 DHCP 租约导致重启后 4G 再次不可用的问题。
- 4G 策略接口的空服务列表固定返回 `[]`，通知器也兼容旧后端的 `null`，修复切换“允许 4G 上网”时提示“未能读取数据，因为数据丢失”。
- 短信页将 SIM 实时返回的号码明确标注为“本机号码”，避免被误认为默认收件人；号码不写入本地配置或草稿，新短信收件人仍默认为空。
- 呼出电话处于 `dialing` / `alerting` 时播放本地回铃音，并在接通、挂断或呼叫结束时立即停止；忽略挂断后短暂回跳的同一通话记录，模块 UAC 语音路由仍只在 `active` 后启动，避免提前抢占真实通话音频。

## 2026-08-30 · v1.2.10（移动设备网络唤醒）

- iPhone/iPad 模式增加模块侧网络唤醒服务，不依赖手机 App：USB 已连接时持有模块 wake lock，并定期维持 USB ECM 与蜂窝数据链路活动。
- 移动设备断开或恢复 Mac 完整模式后自动释放 wake lock，避免无连接时持续耗电。
- 唤醒服务为公开、可审计的 shell 源码；通过 Mac 的 ADB 在切换移动模式时一次性安装，不包含私有模块 Agent。
- 本轮只调整移动模式网络保活，不修改通话语音、短信、4G 开关和 USB 自动切换策略。
